...
class TestVideoMetadataGenerator(unittest.TestCase):
  ...
  def test_get_video_id_from_url(self):
    ...
    self.assertEqual(
        ai_metadata_generator._get_video_id_from_url(url), expected_id
    )

    url_no_match = "https://example.com/video/1234abcd"
    self.assertIsNone(
        ai_metadata_generator._get_video_id_from_url(url_no_match)
    )
  ...
  @patch(
      "builtins.open", new_callable=mock_open, read_data="This is a transcript."
  )
  ...
  def test_get_transcription_from_video(
    ...
    """Tests successfully getting a trancript from a video."""
    ...
    mock_upload_local_file_to_google_cloud.return_value = (
        "gs://audio_bucket/1234.wav"
    )
    mock_transcribe_audio.return_value = (
        "./transcriptions/1234_transcription.txt"
    )

    transcript = ai_metadata_generator._get_transcription_from_video(
        video, audio_bucket_name, transcript_bucket_name
    )
  ...
  @patch(
      "ai_metadata_generator._get_transcription_from_video",
      return_value="""
      This is a long transcript with more than 100 characters. This ensures the
      transcription length condition is satisfied.
      """,
  )
  @patch("llm_utils.call_llm")
  def test_add_ai_attributes_to_video(
      self, mock_call_llm, mock_get_transcription_from_video
  ):
    ...
    ai_metadata_generator._add_ai_attributes_to_video(
        video, audio_bucket_name, transcript_bucket_name
    )
  ...
  @patch(
      "ai_metadata_generator._get_transcription_from_video",
      return_value="This is a video longer than 40 minutes.",
  )
  @patch("llm_utils.call_llm")
  ...
  @patch(
      "ai_metadata_generator._get_video_id_from_url", return_value="1234abcd"
  )
  def test_main(
      self, mock_get_video_id_from_url, mock_add_ai_attributes_to_video
  ):
    ...
    mock_add_ai_attributes_to_video.assert_called_once_with(
        result_video, audio_bucket_name, transcript_bucket_name
    )
